...
if not headless:
    ...
    if freqDomain:
        ...
        plt.grid()
        lineFreq, = axFreq.plot(range(int(NFFT/2)), [0]*(int(NFFT/2)),c='#6baed6', label='FFT')
        if lpcOverlay:
            lineLPC, = axFreq.plot(range(int(NFFT/2)), [0]*(int(NFFT/2)),'--', c='#08519c', label='LPC Envelope')
        plt.ylim([-50, 300])
        plt.xlim([0, NFFT/2])
